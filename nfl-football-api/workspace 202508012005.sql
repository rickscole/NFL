...
select e.player_id, e.game_id, e.stat_type, stat_value = try_convert(decimal(18, 6), e.stat_value) from [temp].[nfl_nfapi_player_game_stats_stacked] e union all
select d.* from (
...
, a.stat_type
, stat_value
, stat_value_01 = try_convert(decimal(12, 6), a.stat_value)
from [temp].[nfl_nfapi_player_game_stats_stacked] a 
)
b 
where stat_value_01 is null
group by stat_type

select stat_type, count(*) from [temp].[nfl_nfapi_player_game_stats_stacked] group by stat_type
*/
